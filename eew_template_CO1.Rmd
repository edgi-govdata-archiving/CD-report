...
filenames = list.files(here("CO1"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("CO1", i)))
}

activefacilities <- `active-facilities_All_pg3_CO-1`
...
```
 <div class="container">
  <img src="CO1_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">Colorado's 1st District</div>
  <div class="middle-left">Seat held by Diana Degette since 1997</div>
</div>
...
# Highlights for Colorado's 1st District
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_CO-1`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CO-1`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_CO-1`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CO 1
CAArecurring <- `recurring-violations_All_pg3_CO-1`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CO 1")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CO 1")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in CO1, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_CO-082720.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CO-082720.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_CO-1`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CO-1`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CO-082720.csv`, `inspectionsper1000_All_pg4_CO-1`)
...
names(`violationsper1000_All_pg4_CO-082720.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CO-082720.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_CO-1`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CO-1`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CO-082720.csv`, `violationsper1000_All_pg4_CO-1`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CO-1` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CO-1`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CO-1`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_CO-1`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CO-1`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CO-1`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CO-1`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CO-1`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CO-1`
...
GHGs <- `emissions2018_GHGRP_pg4_CO-1`
#GHGs_state <- `emissions2018_GHGRP_pg4_CO-082720.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "CO 1 Emissions"
names(GHGs)[3] <- "CO Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`CO Emissions`-GHGs$`CO 1 Emissions`)
GHGs$`CO 1 Emissions` <- (GHGs$`CO 1 Emissions`/100000)
GHGs$`CO 1 Emissions` <- round(GHGs$`CO 1 Emissions`,2)
GHGs$`OR Emissions` <- (GHGs$`CO Emissions`/100000)
GHGs$`OR Emissions` <- round(GHGs$`CO Emissions`,2)
...
GHG_CD$Region <- "CO"
GHG_CO <- GHGs2[c(1,3)]
names(GHG_CO)[2] <- "Emissions"
GHG_CO$Region <- "CO 1"
GHGs_forgraph <- rbind(GHG_CO, GHG_CD)
...
<a ><img src="diana_degette.png" style="float: left; width: 25%; margin-right: 5%" ></a>
...
2. Chair - [Oversights and Investigations](https://energycommerce.house.gov/subcommittees/oversight-and-investigations-116th-congress)
