...
filenames = list.files(here("MA4_2"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("MA4_2", i)))
}
...
```
 <div class="container">
  ...
  <div class="top-left">Massachusetts' 4th District</div>
  <div class="middle-left">Seat held by Joseph Kennedy III since 2013</div>
</div>
### 26 August 2020  
...
# Highlights for Massachusetts' 4th District
...
inspectiongraph <- ggplot(inspections, mapping=aes(x=factor(Date),
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0,150),breaks=c(0,50,100,150))+
  scale_x_discrete(breaks=c(2004,2008,2012,2016,2019))+
  theme_meg()+
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in MA4, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_MA-082620.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_MA-082620.csv`$Region <- as.factor("State")
...
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_MA-082620.csv`, `inspectionsper1000_All_pg4_MA-4`)
...
names(`violationsper1000_All_pg4_MA-082620.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_MA-082620.csv`$Region <- as.factor("State")
...
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_MA-082620.csv`, `violationsper1000_All_pg4_MA-4`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_MA-4` 
CAAviolations$President <- as.character("Bush")
CAAviolations[9:14,3] <- as.character("Obama")
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_MA-4`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_MA-4`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_MA-4`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_MA-4`
#CWAviolations <- CWAviolations[-c(20),]
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_MA-4`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_MA-4`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_MA-4`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_MA-4`
...
GHGs <- `emissions2018_GHGRP_pg4_MA-4`
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "MA 4 Emissions"
names(GHGs)[3] <- "MA Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`MA Emissions`-GHGs$`MA 4 Emissions`)
GHGs$`MA 4 Emissions` <- (GHGs$`MA 4 Emissions`/100000)
GHGs$`MA 4 Emissions` <- round(GHGs$`MA 4 Emissions`,2)
GHGs$`MA Emissions` <- (GHGs$`MA Emissions`/100000)
GHGs$`MA Emissions` <- round(GHGs$`MA Emissions`,2)
...
GHG_CD$Region <- "MA 4"
GHG_MA <- GHGs2[c(1,3)]
names(GHG_MA)[2] <- "Emissions"
GHG_MA$Region <- "MA"
GHGs_forgraph <- rbind(GHG_MA, GHG_CD)
