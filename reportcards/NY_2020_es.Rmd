...
filenames = list.files(here("../CD_Dirs/NY"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  # name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/NY", i)))
}
...
##### El estado de New York; asiento ocupado por `r NY[1,2]` desde `r NY[1,8]` 
 <div class="container">
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="NY", "NY", "otros CDs/\nestados")
...
#removing NY9 and WA5 and MI7, NY, and IA2 and extra CA9
CWAper100 <- CWAper100[-c(37,51,28,14,53,8),]

NY_dot <- CWAper1000[which(CWAper1000$CD.State=="NY"), ]

##THIS CODE WILL NOT WORK FOR NY9 and WA5 and MI7, NY, and IA2. Omit this chunk of code for those districts (here to the end of this r chunk)
...
ggsave("dotplot_NY.png", dotplot, path=here("../CD_Dirs/NY"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/NY/dotplot_NY.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En NY, hubo <span style="color:#569b5e">`r NY_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
```
# ASPECTOS DESTACADOS DEL  ESTADO DE NEW YORK
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/NY"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/NY"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/NY"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NY/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NY/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NY/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
## Ha estado en esta oficina `r NY[1,6]` `r NY[1,7]`, `r NY[1,8]`
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/NY_2020.pdf)
