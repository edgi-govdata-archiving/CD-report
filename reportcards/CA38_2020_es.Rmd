...
filenames = list.files(here("../CD_Dirs/CA38"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/CA38", i)))
}

activefacilities <- `active-facilities_All_pg3_CA-38`
...
##### El 38$^\circ$ distrito de California; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/CA38_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_CA-38`
...
enforcement <- `enforcements_All_pg3_CA-38`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="CA38", "CA38", "otros CDs/\nestados")
...
CA38_dot <- CWAper1000[which(CWAper1000$CD.State=="CA38"), ]
...
ggsave("dotplot_CA38.png", dotplot, path=here("../CD_Dirs/CA38"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/CA38/dotplot_CA38.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En CA38, hubo <span style="color:#569b5e">`r CA38_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CA 38
CAArecurring <- `recurring-violations_All_pg3_CA-38`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CA 38")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CA 38")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_CA-38`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CA-38`
...
```
# ASPECTOS DESTACADOS DEL 38$^\circ$ DISTRITO DE CALIFORNIA
...
names(`inspectionsper1000_All_pg4_CA-102320.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-102320.csv`$Region <- as.factor("Este Estado")
names(`inspectionsper1000_All_pg4_CA-38`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-38`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA-102320.csv`, `inspectionsper1000_All_pg4_CA-38`)
...
names(`violationsper1000_All_pg4_CA-102320.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-102320.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_CA-38`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-38`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA-102320.csv`, `violationsper1000_All_pg4_CA-38`)
...
CAAbadactors <- `noncomp_CAA_pg6_CA-38`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/CA38"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_CA-38`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/CA38"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_CA-38`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/CA38"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/CA38/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/CA38/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/CA38/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CA-38` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CA-38`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CA-38`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CA-38`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CA-38`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CA-38`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CA-38`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CA-38`
...
<br />
<a ><img src="../CD_images/CA38_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
## Nombre del comité: Comité de la Cámara de Medios y Arbitrios 
Jurisdicción: El Comité de Medios y Arbitrios es el comité principal de redacción de impuestos en la Cámara de Representantes. El Comité obtiene una gran parte de su jurisdicción del Artículo I, Sección VII de la Constitución de los EE. UU., Que declara, “Todos los proyectos de ley para recaudar ingresos deben venir de la Cámara de Representantes ". El comité tiene jurisdicción sobre los ingresos y sobre asuntos relacionados, como las tarifas, los acuerdos comerciales recíprocos y la deuda en bonos de los Estados Unidos. También supervisa los aspectos relacionados con los ingresos del sistema de Seguridad Social, Medicare y los programas de servicios sociales.
<br />
[Página web del comité](https://waysandmeans.house.gov/)
<br /> <br /> 

### Subcomité: Seguridad Social, Rango: 3  <br /> 

### Subcomité: Seleccionar medidas de ingresos, Rango: 4

### Subcomité: Supervisión, Rango: 3 <br />
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/CA38_2020.pdf)
