...
filenames = list.files(here("../CD_Dirs/TX20"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/TX20", i)))
}

activefacilities <- `active-facilities_All_pg3_TX-20`
...
##### El 20$^\circ$ distrito de Texas; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/TX20_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_TX-20`
...
enforcement <- `enforcements_All_pg3_TX-20`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="TX20", "TX20", "otros CDs/\nestados")
...
TX20_dot <- CWAper1000[which(CWAper1000$CD.State=="TX20"), ]
...
ggsave("dotplot_TX20.png", dotplot, path=here("../CD_Dirs/TX20"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/TX20/dotplot_TX20.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En TX20, hubo <span style="color:#569b5e">`r TX20_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for TX 20
CAArecurring <- `recurring-violations_All_pg3_TX-20`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("TX 20")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("TX 20")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_TX-20`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_TX-20`
...
```
# ASPECTOS DESTACADOS DEL 20$^\circ$ DISTRITO DE TEXAS
...
names(`inspectionsper1000_All_pg4_TX-20`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_TX-20`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_TX-112920.csv`, `inspectionsper1000_All_pg4_TX-20`)
...
names(`violationsper1000_All_pg4_TX-20`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_TX-20`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_TX-112920.csv`, `violationsper1000_All_pg4_TX-20`)
...
CAAbadactors <- `noncomp_CAA_pg6_TX-20`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/TX20"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_TX-20`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/TX20"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_TX-20`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/TX20"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX20/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX20/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX20/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_TX-20` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_TX-20`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_TX-20`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_TX-20`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_TX-20`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_TX-20`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_TX-20`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_TX-20`
...
<br />
<a ><img src="../CD_images/TX20_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2>[`r leg_info$name`](`r leg_info$wikipedia_url`) (Demócrata)</h2>

## Ha estado en esta oficina desde el enero 3 del 2013 
...
Jurisdicción: El Comité Permanente de Inteligencia de la Cámara de Representantes de los Estados Unidos (HPSCI) es un comité de la Cámara de Representantes de los Estados Unidos, actualmente presidido por el congresista Devin Nunes (California). Creada en 1977, la HPSCI está a cargo de la supervisión de la Comunidad de Inteligencia de los Estados Unidos, que incluye las actividades de inteligencia y las actividades relacionadas con la inteligencia de los siguientes diecisiete elementos del gobierno de los Estados Unidos, y el Programa de Inteligencia Militar. 
<br />
[Página web del comité](https://intelligence.house.gov/)
<br /> <br /> 

### Subcomité: Contraterrorismo, Contrainteligencia y Contra proliferación, Rango: 4 <br /> 

### Subcomité: Modernización de la Inteligencia y Preparación, Rango: 4

<br />

## Nombre del comité: Comité de Educación y Trabajo de la Cámara de Representantes 
Jurisdicción: El comité tiene jurisdicción legislativa sobre asuntos relacionados con la educación superior e inferior, el desarrollo y la protección de la fuerza laboral, la salud, el empleo, el trabajo y las pensiones. 
<br />
[Página web del comité](https://edlabor.house.gov/)
<br /> <br /> 

### Subcomité: Inversión en Educación Superior y Fuerza Laboral, Rango: 11 <br />

<br />  

## Nombre del comité: Comité de Relaciones Exteriores de la Cámara de Representantes 
Jurisdicción: El Comité de Relaciones Exteriores de la Cámara de Representantes considera la legislación que afecta a la comunidad diplomática, que incluye al Departamento de Estado, la Agencia para el Desarrollo Internacional, el PeaceCorps las Naciones Unidas y la aplicación de la Ley de Control de Exportación de Armas. 
<br />
[Página web del comité](https://foreignaffairs.house.gov/)
<br /> <br /> 

### Subcomité: Hemisferio Occidental, Seguridad Civil y Comercio, Rango: 3 <br />

### Subcomité: Supervisión e Investigaciones, Rango: 1
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/TX20_2020.pdf)
