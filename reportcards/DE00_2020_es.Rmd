...
filenames = list.files(here("../CD_Dirs/DE"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/DE", i)))
  # cat( "file is ", name, "\n" )
}
...
##### El  distrito de Delaware; asiento ocupado por `r DE[1,2]` desde `r DE[1,8]` 
 <div class="container">
  <img src="../CD_maps/DE_map.png" alt="Snow" style="width:100%;">
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="DE", "DE", "otros CDs/\nestados")
...
ggsave("dotplot_DE.png", dotplot, path=here("../CD_Dirs/DE"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/DE/dotplot_DE.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En DE, hubo <span style="color:#569b5e">`r DE_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for DE 0
...
CAArecurring$Region2 <- as.character("All of DE")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for DE 0
CWArecurring <- `recurring-violations_All_pg3_DE`[-c(1,3,4,6),]
CWArecurring <- CWArecurring %>%
  select(State,Facilities, Percent)
names(CWArecurring)[1] <- "Region"
CWArecurring[1,1] <- as.character("Rest of State")
CWArecurring[2,1] <- as.character("DE")
CWArecurring$Region2 <- as.character("All of DE")
CWArecurring[2,4] <- as.character("DE")
...
RCRArecurring$Region2 <- as.character("All of DE")
...
```
# ASPECTOS DESTACADOS DEL DISTRITO DE Delaware
...
names(`inspectionsper1000_All_pg4_DE`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_DE`$Region <- as.factor("Congressional District")
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/DE"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/DE"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/DE"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/DE/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/DE/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/DE/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
## Ha estado en esta oficina `r DE[1,6]` `r DE[1,7]`, `r DE[1,8]`
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/DE_2020.pdf)
