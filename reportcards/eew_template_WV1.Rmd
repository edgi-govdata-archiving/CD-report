...
for (i in filenames){  
  ...
  assign(name, read.csv(here("WV1", i)))
}

activefacilities <- `active-facilities_All_pg3_WV-1`
...
```
 <div class="container">
  ...
  <div class="top-left">West Virginia's 1st District</div>
  <div class="middle-left">Seat held by David McKinley since 2011</div>
</div>
...
# Highlights for West Virginia's 1st District
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_WV-1`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_WV-1`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_WV-1`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for WV 1
CAArecurring <- `recurring-violations_All_pg3_WV-1`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("WV 1")
CAArecurring$Region2 <- as.character("All of WV")
CAArecurring[2,4] <- as.character("WV 1")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for WV 1
CWArecurring <- `recurring-violations_All_pg3_WV-1`[-c(1,3,4,6),]
CWArecurring <- CWArecurring %>%
  select(CD,Facilities, Percent)
names(CWArecurring)[1] <- "Region"
CWArecurring[1,1] <- as.character("Rest of State")
CWArecurring[2,1] <- as.character("WV 1")
CWArecurring$Region2 <- as.character("All of WV")
CWArecurring[2,4] <- as.character("WV 1")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for WV 1
RCRArecurring <- `recurring-violations_All_pg3_WV-1`[-c(1,2,4,5),]
RCRArecurring <- RCRArecurring %>%
  select(CD,Facilities, Percent)
names(RCRArecurring)[1] <- "Region"
RCRArecurring[1,1] <- as.character("Rest of State")
RCRArecurring[2,1] <- as.character("WV 1")
RCRArecurring$Region2 <- as.character("All of WV")
RCRArecurring[2,4] <- as.character("WV 1")
...
names(`inspectionsper1000_All_pg4_CO-090220.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CO-090220.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_WV-1`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_WV-1`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CO-090220.csv`, `inspectionsper1000_All_pg4_WV-1`)
...
names(`violationsper1000_All_pg4_CO-090220.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CO-090220.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_WV-1`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_WV-1`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CO-090220.csv`, `violationsper1000_All_pg4_WV-1`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_WV-1` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_WV-1`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_WV-1`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_WV-1`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_WV-1`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_WV-1`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_WV-1`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_WV-1`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_WV-1`
...
GHGs <- `emissions2018_GHGRP_pg4_WV-1`
#GHGs_state <- `emissions2018_GHGRP_pg4_CO-090220.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "WV 1 Emissions"
names(GHGs)[3] <- "WV Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`WV Emissions`-GHGs$`WV 1 Emissions`)
GHGs$`WV 1 Emissions` <- (GHGs$`WV 1 Emissions`/100000)
GHGs$`WV 1 Emissions` <- round(GHGs$`WV 1 Emissions`,2)
GHGs$`WV Emissions` <- (GHGs$`WV Emissions`/100000)
GHGs$`WV Emissions` <- round(GHGs$`WV Emissions`,2)
...
GHG_CD$Region <- "WV"
GHG_WV <- GHGs2[c(1,3)]
names(GHG_WV)[2] <- "Emissions"
GHG_WV$Region <- "WV 1"
GHGs_forgraph <- rbind(GHG_WV, GHG_CD)
...
<a ><img src="" style="float: left; width: 25%; margin-right: 5%" ></a>
