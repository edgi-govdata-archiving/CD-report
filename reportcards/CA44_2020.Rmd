...
filenames = list.files(here("CD_Dirs/CA44"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/CA44", i)))
}

activefacilities <- `active-facilities_All_pg3_CA-44`
...
CA44 <- housemembers[which(housemembers$CD=="CA44"), ]
...
##### California's 44th District; seat held by Nanette Diaz Barragán since `r CA44[1,8]` 
 <div class="container">
...
CWAviolations <- `violations_CWA_pg6_CA-44`
...
enforcement <- `enforcements_All_pg3_CA-44`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="CA44", "CA44", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="CA44", "CA44", "Other")
...
CA44_dot <- CWAper1000[which(CWAper1000$CD.State=="CA44"), ]
...
ggsave("dotplot_CA44.png", dotplot, path=here("CD_Dirs/CA44"), scale = 2, width=8, height=5, units=c("cm"))
...
<h3>In CA44, there were <span style="color:#569b5e">`r CA44_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CA 44
CAArecurring <- `recurring-violations_All_pg3_CA-44`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CA 44")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CA 44")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_CA-44`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CA-44`
...
names(`inspectionsper1000_All_pg4_CA-44`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-44`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA-091820.csv`, `inspectionsper1000_All_pg4_CA-44`)
...
inspect1000bar <- ggplot(inspectionsper1000, aes(fill=Region, y=Inspectionsper1000, x=Program, alpha=Program))+
  ...
  scale_y_continuous(expand=c(0,0),limits=c(0,2000))+geom_hline(yintercept=1000, linetype="dashed", color="black")+geom_text(aes(label="Average of 1 inspection per facility",family="Georgia"),x=2,y=1050,color="black",size=4)+
  theme_meg()
...
names(`violationsper1000_All_pg4_CA-44`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-44`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA-091820.csv`, `violationsper1000_All_pg4_CA-44`)
...
CAAbadactors <- `noncomp_CAA_pg6_CA-44`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_CA-44`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_CA-44`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/CA44"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/CA44"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/CA44"), scale = 3, width=7, height=4, units=c("cm"))
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CA-44` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CA-44`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CA-44`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CA-44`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CA-44`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CA-44`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CA-44`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CA-44`
...
<h2>[Nanette Diaz Barragán](`r CA44[1,9]`) (`r CA44[1,3]`)</h2>

## In office since `r CA44[1,6]` `r CA44[1,7]`, `r CA44[1,8]`
...
<h4><span style="padding-left:30px">[`r CA44[1,10]`](`r CA44[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA44[1,12]`](`r CA44[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA44[1,14]`](`r CA44[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA44[1,16]`](`r CA44[1,17]`)</span></h4>
