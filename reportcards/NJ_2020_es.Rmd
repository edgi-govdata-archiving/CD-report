...
filenames = list.files(here("../CD_Dirs/NJ"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  # name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/NJ", i)))
}

activefacilities <- `active-facilities_All_pg3_NJ`
...
NJ <- senators[which(senators$state=="NJ"), ]
```
...
##### El estado de New Jersey; asiento ocupado por `r NJ[1,2]` desde `r NJ[1,8]` 
 <div class="container">
...
CWAviolations <- `violations_CWA_pg6_NJ`
...
enforcement <- `enforcements_All_pg3_NJ`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="NJ", "NJ", "otros CDs/\nestados")
...
#removing NY9 and WA5 and MI7, NJ, and IA2 and extra CA9
CWAper100 <- CWAper100[-c(37,51,28,14,53,8),]

NJ_dot <- CWAper1000[which(CWAper1000$CD.State=="NJ"), ]

##THIS CODE WILL NOT WORK FOR NY9 and WA5 and MI7, NJ, and IA2. Omit this chunk of code for those districts (here to the end of this r chunk)
...
ggsave("dotplot_NJ.png", dotplot, path=here("../CD_Dirs/NJ"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/NJ/dotplot_NJ.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En NJ, hubo <span style="color:#569b5e">`r NJ_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NJ
CAArecurring <- `recurring-violations_All_pg3_NJ`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NJ")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("NJ")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for NJ
CWArecurring <- `recurring-violations_All_pg3_NJ`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_NJ`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NJ`
...
```
# ASPECTOS DESTACADOS DEL  ESTADO DE NEW JERSEY
...
names(`inspectionsper1000_All_pg4_NJ`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NJ`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NJ`)
...
names(`violationsper1000_All_pg4_NJ`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NJ`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NJ`)
...
CAAbadactors <- `noncomp_CAA_pg6_NJ`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/NJ"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_NJ`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/NJ"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_NJ`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/NJ"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NJ/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NJ/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/NJ/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_NJ` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_NJ`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NJ`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NJ`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NJ`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_NJ`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NJ`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NJ`
...
<h2>[`r NJ[1,2]`](`r NJ[1,9]`) (`r NJ[1,3]`)</h2>

## Ha estado en esta oficina `r NJ[1,6]` `r NJ[1,7]`, `r NJ[1,8]`
...
<h4><span style="padding-left:30px">[`r NJ[1,10]`](`r NJ[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r NJ[1,12]`](`r NJ[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r NJ[1,14]`](`r NJ[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r NJ[1,16]`](`r NJ[1,17]`)</span></h4>
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/NJ_2020.pdf)
