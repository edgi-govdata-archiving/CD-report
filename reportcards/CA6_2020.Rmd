...
filenames = list.files(here("CD_Dirs/CA6"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/CA6", i)))
}

activefacilities <- `active-facilities_All_pg3_CA-6`
...
CA6 <- housemembers[which(housemembers$CD=="CA6"), ]
...
##### California's 6th District; seat held by `r CA6[1,2]` since `r CA6[1,8]` 
 <div class="container">
...
CWAviolations <- `violations_CWA_pg6_CA-6`
...
enforcement <- `enforcements_All_pg3_CA-6`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="CA6", "CA6", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="CA6", "CA6", "Other")
...
CA6_dot <- CWAper1000[which(CWAper1000$CD.State=="CA6"), ]
...
ggsave("dotplot_CA6.png", dotplot, path=here("CD_Dirs/CA6"), scale = 2, width=8, height=5, units=c("cm"))
...
<h3>In CA6, there were <span style="color:#569b5e">`r CA6_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CA 6
CAArecurring <- `recurring-violations_All_pg3_CA-6`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CA 6")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CA 6")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_CA-6`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CA-6`
...
names(`inspectionsper1000_All_pg4_CA-6`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-6`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA-091820.csv`, `inspectionsper1000_All_pg4_CA-6`)
...
names(`violationsper1000_All_pg4_CA-6`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-6`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA-091820.csv`, `violationsper1000_All_pg4_CA-6`)
...
CAAbadactors <- `noncomp_CAA_pg6_CA-6`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_CA-6`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_CA-6`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/CA6"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/CA6"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/CA6"), scale = 3, width=7, height=4, units=c("cm"))
...
<a ><img src="../CD_Dirs/CA6/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CA-6` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CA-6`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CA-6`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CA-6`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CA-6`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CA-6`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CA-6`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CA-6`
...
<h2>[`r CA6[1,2]`](`r CA6[1,9]`) (`r CA6[1,3]`)</h2>

## In office since `r CA6[1,6]` `r CA6[1,7]`, `r CA6[1,8]`
...
<h4><span style="padding-left:30px">[`r CA6[1,10]`](`r CA6[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA6[1,12]`](`r CA6[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA6[1,14]`](`r CA6[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA6[1,16]`](`r CA6[1,17]`)</span></h4>
