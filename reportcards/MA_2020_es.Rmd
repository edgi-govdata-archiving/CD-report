...
filenames = list.files(here("../CD_Dirs/MA"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  # name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/MA", i)))
}
...
##### El estado de Massachusetts; asiento ocupado por `r MA[1,2]` desde `r MA[1,8]` 
 <div class="container">
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="MA", "MA", "otros CDs/\nestados")
...
#removing NY9 and WA5 and MI7, MA, and IA2 and extra CA9
CWAper100 <- CWAper100[-c(37,51,28,14,53,8),]

MA_dot <- CWAper1000[which(CWAper1000$CD.State=="MA"), ]

##THIS CODE WILL NOT WORK FOR NY9 and WA5 and MI7, MA, and IA2. Omit this chunk of code for those districts (here to the end of this r chunk)
...
ggsave("dotplot_MA.png", dotplot, path=here("../CD_Dirs/MA"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/MA/dotplot_MA.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En MA, hubo <span style="color:#569b5e">`r MA_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
```
# ASPECTOS DESTACADOS DEL  ESTADO DE MASSACHUSETTS
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/MA"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/MA"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/MA"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/MA/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/MA/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/MA/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
## Ha estado en esta oficina `r MA[1,6]` `r MA[1,7]`, `r MA[1,8]`
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/MA_2020.pdf)
