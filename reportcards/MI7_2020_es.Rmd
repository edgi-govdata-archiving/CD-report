...
filenames = list.files(here("../CD_Dirs/MI7"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/MI7", i)))
}

activefacilities <- `active-facilities_All_pg3_MI-7`
...
MI7 <- housemembers[which(housemembers$CD=="MI7"), ]
...
##### El 7$^\circ$ distrito de Michigan; asiento ocupado por `r MI7[1,2]` desde `r MI7[1,8]` 
 <div class="container">
  <img src="../CD_maps/MI7_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_MI-7`
...
enforcement <- `enforcements_All_pg3_MI-7`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="MI7", "MI7", "otros CDs/\nestados")
...
MI7_dot <- CWAper1000[which(CWAper1000$CD.State=="MI7"), ]
...
ggsave("dotplot_MI7.png", dotplot, path=here("../CD_Dirs/MI7"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/MI7/dotplot_MI7.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En MI7, hubo <span style="color:#569b5e">`r MI7_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for MI 7
CAArecurring <- `recurring-violations_All_pg3_MI-7`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("MI 7")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("MI 7")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_MI-7`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_MI-7`
...
```
# ASPECTOS DESTACADOS DEL 7$^\circ$ DISTRITO DE MICHIGAN
...
names(`inspectionsper1000_All_pg4_MI-7`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_MI-7`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_MI-091820.csv`, `inspectionsper1000_All_pg4_MI-7`)
...
names(`violationsper1000_All_pg4_MI-7`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_MI-7`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_MI-091820.csv`, `violationsper1000_All_pg4_MI-7`)
...
CAAbadactors <- `noncomp_CAA_pg6_MI-7`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/MI7"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_MI-7`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/MI7"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_MI-7`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/MI7"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/MI7/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/MI7/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/MI7/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_MI-7` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_MI-7`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_MI-7`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_MI-7`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_MI-7`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_MI-7`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_MI-7`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_MI-7`
...
<br />
<a ><img src="../CD_images/MI7_rep.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r MI7[1,2]`](`r MI7[1,9]`) (`r MI7[1,3]`)</h2>

## Ha estado en esta oficina `r MI7[1,6]` `r MI7[1,7]`, `r MI7[1,8]`
...
<h4><span style="padding-left:30px">[`r MI7[1,10]`](`r MI7[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MI7[1,12]`](`r MI7[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MI7[1,14]`](`r MI7[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MI7[1,16]`](`r MI7[1,17]`)</span></h4>
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/MI7_2020.pdf)
