...
filenames = list.files(here("CD_Dirs/CA29"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/CA29", i)))
}

activefacilities <- `active-facilities_All_pg3_CA-29`
...
CA29 <- housemembers[which(housemembers$CD=="CA29"), ]
...
##### California's 29th District; seat held by Tony Cárdenas since `r CA29[1,8]` 
 <div class="container">
...
CWAviolations <- `violations_CWA_pg6_CA-29`
...
<h3>Clean Water Act Violations*: <span style="color:#569b5e">`r vCWAchange[1,4]` `r vCWAchange[1,5]`</span> than the average over the previous 16 years, representing a <span style="color:#569b5e">24,821% `r vCWAchange[1,7]`</span> in violations</h3>
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="CA29", "CA29", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="CA29", "CA29", "Other")
...
CA29_dot <- CWAper1000[which(CWAper1000$CD.State=="CA29"), ]
...
ggsave("dotplot_CA29.png", dotplot, path=here("CD_Dirs/CA29"), scale = 2, width=8, height=5, units=c("cm"))
...
<h3>In CA29, there were <span style="color:#569b5e">`r CA29_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CA 29
CAArecurring <- `recurring-violations_All_pg3_CA-29`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CA 29")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CA 29")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_CA-29`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CA-29`
...
names(`inspectionsper1000_All_pg4_CA-29`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-29`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA-091820.csv`, `inspectionsper1000_All_pg4_CA-29`)
...
names(`violationsper1000_All_pg4_CA-29`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-29`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA-091820.csv`, `violationsper1000_All_pg4_CA-29`)
...
CAAbadactors <- `noncomp_CAA_pg6_CA-29`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_CA-29`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_CA-29`
...
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/CA29"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/CA29"), scale = 3, width=7, height=4, units=c("cm"))
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CA-29` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CA-29`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CA-29`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CA-29`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CA-29`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CA-29`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CA-29`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CA-29`
...
<h2>[Tony Cárdenas](`r CA29[1,9]`) (`r CA29[1,3]`)</h2>

## In office since `r CA29[1,6]` `r CA29[1,7]`, `r CA29[1,8]`
...
<h4><span style="padding-left:30px">[`r CA29[1,10]`](`r CA29[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA29[1,12]`](`r CA29[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA29[1,14]`](`r CA29[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA29[1,16]`](`r CA29[1,17]`)</span></h4>
