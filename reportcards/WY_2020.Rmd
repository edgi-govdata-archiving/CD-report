...
filenames = list.files(here("CD_Dirs/WY"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/WY", i)))
}

activefacilities <- `active-facilities_All_pg3_WY`
...
WY <- housemembers[which(housemembers$CD=="WY"), ]
...
##### Wyoming; seat held by `r WY[1,2]` since `r WY[1,8]` 
 <div class="container">
  <img src="../CD_maps/WY_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- `violations_CWA_pg6_WY`
...
enforcement <- `enforcements_All_pg3_WY`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="WY", "WY", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="WY", "WY", "Other")
...
WY_dot <- CWAper1000[which(CWAper1000$CD.State=="WY"), ]
...
ggsave("dotplot_WY.png", dotplot, path=here("CD_Dirs/WY"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/WY/dotplot_WY.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In WY, there were <span style="color:#569b5e">`r WY_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for WY
CAArecurring <- `recurring-violations_All_pg3_WY`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("WY")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("WY")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_WY`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_WY`
...
names(`inspectionsper1000_All_pg4_WY-091820.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_WY-091820.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_WY`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_WY`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_WY-091820.csv`, `inspectionsper1000_All_pg4_WY`)
...
names(`violationsper1000_All_pg4_WY-091820.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_WY-091820.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_WY`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_WY`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_WY-091820.csv`, `violationsper1000_All_pg4_WY`)
...
CAAbadactors <- `noncomp_CAA_pg6_WY`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_WY`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_WY`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/WY"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/WY"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/WY"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../CD_Dirs/WY/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/WY/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/WY/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_WY` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_WY`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_WY`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_WY`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_WY`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_WY`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_WY`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_WY`
...
<br />
<a ><img src="../CD_images/WY_rep.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r WY[1,2]`](`r WY[1,9]`) (`r WY[1,3]`)</h2>

## In office since `r WY[1,6]` `r WY[1,7]`, `r WY[1,8]`
...
<h4><span style="padding-left:30px">[`r WY[1,10]`](`r WY[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WY[1,12]`](`r WY[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WY[1,14]`](`r WY[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WY[1,16]`](`r WY[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/WY_2020.pdf)
