...
filenames = list.files(here("CA9"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CA9", i)))
}
...
```
 <div class="container">
  <img src="../CA9/CA9_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">California's 9th District</div>
  <div class="middle-left">Seat held by Jerry McNerney since 2013</div>
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_CA-9`
...
enforcementfinegraph <- enforcement %>% ggplot()+
  ...
  theme(axis.text.x=element_text(size=9),axis.title.x = element_blank())
```
# Highlights for California's 3rd District
...
* <h2>Under the Clean Water Act, the only well-reported regulation in this report, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in CA9, were in violation for at least 25% of the last 3 years.</h2>
...
names(`inspectionsper1000_All_pg4_CA-090820.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-090820.csv`$Region <- as.factor("State")
...
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA-090820.csv`, `inspectionsper1000_All_pg4_CA-9`)
...
names(`violationsper1000_All_pg4_CA-090820.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-090820.csv`$Region <- as.factor("State")
...
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA-090820.csv`, `violationsper1000_All_pg4_CA-9`)
...
```{r, fig.height = 5, fig.width = 8, fig.align = "center"}
#clean up data
#CWAviolations <- read.csv(here("CA9/violations_CWA_pg6_CA-9-090820.csv"), 
                          #header=FALSE)
...
<a ><img src="../CA9/jerry_mcnerney.png" style="float: left; width: 25%; margin-right: 5%" ></a>
