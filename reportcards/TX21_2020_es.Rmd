...
filenames = list.files(here("../CD_Dirs/TX21"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/TX21", i)))
}

activefacilities <- `active-facilities_All_pg3_TX-21`
...
##### El 21$^\circ$ distrito de Texas; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/TX21_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_TX-21`
...
enforcement <- `enforcements_All_pg3_TX-21`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="TX21", "TX21", "otros CDs/\nestados")
...
TX21_dot <- CWAper1000[which(CWAper1000$CD.State=="TX21"), ]
...
ggsave("dotplot_TX21.png", dotplot, path=here("../CD_Dirs/TX21"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/TX21/dotplot_TX21.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En TX21, hubo <span style="color:#569b5e">`r TX21_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for TX 21
CAArecurring <- `recurring-violations_All_pg3_TX-21`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("TX 21")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("TX 21")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_TX-21`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_TX-21`
...
```
# ASPECTOS DESTACADOS DEL 21$^\circ$ DISTRITO DE TEXAS
...
`inspectionsper1000_All_pg4_TX-112920.csv`$Region <- as.factor("Este Estado")
names(`inspectionsper1000_All_pg4_TX-21`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_TX-21`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_TX-112920.csv`, `inspectionsper1000_All_pg4_TX-21`)
...
names(`violationsper1000_All_pg4_TX-21`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_TX-21`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_TX-112920.csv`, `violationsper1000_All_pg4_TX-21`)
...
CAAbadactors <- `noncomp_CAA_pg6_TX-21`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/TX21"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_TX-21`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/TX21"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_TX-21`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/TX21"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX21/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX21/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX21/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_TX-21` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_TX-21`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_TX-21`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_TX-21`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_TX-21`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_TX-21`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_TX-21`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_TX-21`
...
<br />
<a ><img src="../CD_images/TX21_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2>[`r leg_info$name`](`r leg_info$wikipedia_url`) (Republicano)</h2>
...
## Nombre del comité: Comité de Presupuesto de la Cámara 
Jurisdicción: El Comité de Presupuesto de la Cámara de Representantes es responsable de redactar una resolución concurrente sobre el presupuesto para los gastos, los ingresos y los límites de deuda del Congreso. La Resolución de Presupuesto debe ser aprobada por la Cámara y el Senado, pero no necesita ser firmada por el presidente. El Comité también es responsable de hacer cumplir la Resolución del Presupuesto informando a los senadores de las violaciones al presupuesto. El comité también debe trabajar con otros comités, como el Comité de Asignaciones, para prevenir posibles violaciones. 
[Página web del comité](https://budget.house.gov/)
<br /> <br /> 

## Nombre del comité: Comité de Supervisión y Reforma de la Cámara de Representantes 
Jurisdicción: El comité supervisa el gobierno federal y todas sus agencias para garantizar la eficiencia y la responsabilidad. El Comité supervisa las operaciones gubernamentales, la atención médica, la tecnología de la información, el interior, la energía y el medio ambiente, la seguridad nacional y los asuntos intergubernamentales. 
[Página web del comité](https://oversight.house.gov/)
<br /> <br /> 

### Subcomité: Derechos Civiles y Libertades Civiles, Rango: 1 <br /> 

### Subcomité:  Política Económica y del Consumidor, Rango: 4

<br />

## Comité de Asuntos de Veteranos de la Cámara de Representantes 
Jurisdicción: El Comité de Asuntos de Veteranos de la Cámara es el comité autorizador del Departamento de Asuntos de Veteranos (VA). El comité es responsable de recomendar leyes que amplíen, reduzcan o perfeccionen las leyes existentes relacionadas con los beneficios de los veteranos. El Comité también tiene la responsabilidad de supervisar, lo que significa que monitorea y evalúa las operaciones de la VA. Si el Comité encuentra que VA no está administrando leyes como el Congreso se lo pide, entonces se aborda a través del proceso de audiencia y la legislación. 
[Página web del comité](https://veterans.house.gov/)
<br /> <br /> 

### Subcomité: Supervisión e Investigaciones, Rango: 4 <br />

### Subcomité: Modernización de la tecnología, Rango: 3 <br />
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/TX21_2020.pdf)
