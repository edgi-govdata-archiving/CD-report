...
filenames = list.files(here("CD_Dirs/WA5"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/WA5", i)))
}

activefacilities <- `active-facilities_All_pg3_WA-5`
...
WA5 <- housemembers[which(housemembers$CD=="WA5"), ]
...
##### Washington's 5th District; seat held by `r WA5[1,2]` since `r WA5[1,8]` 
 <div class="container">
  <img src="../CD_maps/WA5_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_WA-5`
...
enforcement <- `enforcements_All_pg3_WA-5`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="WA5", "WA5", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="WA5", "WA5", "Other")
...
WA5_dot <- CWAper1000[which(CWAper1000$CD.State=="WA5"), ]
...
<a ><img src="../CD_Dirs/WA5/dotplot_WA5.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In WA5, there were <span style="color:#569b5e">`r WA5_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for WA 5
CAArecurring <- `recurring-violations_All_pg3_WA-5`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("WA 5")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("WA 5")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_WA-5`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_WA-5`
...
```
# Highlights for Washington's 5th District
...
names(`inspectionsper1000_All_pg4_WA-091820.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_WA-091820.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_WA-5`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_WA-5`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_WA-091820.csv`, `inspectionsper1000_All_pg4_WA-5`)
...
inspect1000bar <- ggplot(inspectionsper1000, aes(fill=Region, y=Inspectionsper1000, x=Program, alpha=Program))+
  ...
  scale_y_continuous(expand=c(0,0),limits=c(0,1700))+geom_hline(yintercept=1000, linetype="dashed", color="black")+geom_text(aes(label="Average of 1 inspection per facility",family="Georgia"),x=2,y=1050,color="black",size=4)+
  theme_meg()
...
names(`violationsper1000_All_pg4_WA-091820.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_WA-091820.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_WA-5`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_WA-5`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_WA-091820.csv`, `violationsper1000_All_pg4_WA-5`)
...
CAAbadactors <- `noncomp_CAA_pg6_WA-5`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_WA-5`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_WA-5`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/WA5"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/WA5"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/WA5"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../CD_Dirs/WA5/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/WA5/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/WA5/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_WA-5` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_WA-5`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_WA-5`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_WA-5`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_WA-5`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_WA-5`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_WA-5`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_WA-5`
...
<br />
<a ><img src="../CD_images/WA5_rep.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r WA5[1,2]`](`r WA5[1,9]`) (`r WA5[1,3]`)</h2>

## In office since `r WA5[1,6]` `r WA5[1,7]`, `r WA5[1,8]`
...
<h4><span style="padding-left:30px">[`r WA5[1,10]`](`r WA5[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WA5[1,12]`](`r WA5[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WA5[1,14]`](`r WA5[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WA5[1,16]`](`r WA5[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/WA5_2020.pdf)
