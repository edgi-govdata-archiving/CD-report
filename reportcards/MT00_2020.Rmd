...
filenames = list.files(here("State_Dirs/MT"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/MT", i)))
}
...
##### Montana's Congressional District; seat held by `r MT[1,2]` since `r MT[1,8]` 
 <div class="container">
  <img src="../State_maps/MT_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="MT00", "MT", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="MT00", "MT", "Other")
...
ggsave("dotplot_MT.png", dotplot, path=here("State_Dirs/MT"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/MT/dotplot_MT.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for MT
CWArecurring <- `recurring-violations_All_pg3_MT`[-c(1,3),]
...
`inspectionsper1000_All_pg4_MT`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_MT`)
...
`violationsper1000_All_pg4_MT`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_MT`)
...
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/MT"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/MT"), scale = 3, width=7, height=4, units=c("cm"))
...
<a ><img src="../State_Dirs/MT/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/MT/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/MT00_2020.pdf)
