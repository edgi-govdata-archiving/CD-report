...
filenames = list.files(here("OK2"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("OK2", i)))
}

activefacilities <- `active-facilities_All_pg3_OK-2`
...
```
 <div class="container">
  <img src="OK2_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">Oklahoma's 2nd District</div>
  <div class="middle-left">Seat held by Markwayne Mullin since 2013</div>
</div>
...
# Highlights for Oklahoma's 2nd District
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_OK-2`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_OK-2`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_OK-2`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for OK 2
CAArecurring <- `recurring-violations_All_pg3_OK-2`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("OK 2")
CAArecurring$Region2 <- as.character("All of OK")
CAArecurring[2,4] <- as.character("OK 2")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for OK 2
CWArecurring <- `recurring-violations_All_pg3_OK-2`[-c(1,3,4,6),]
CWArecurring <- CWArecurring %>%
  select(CD,Facilities, Percent)
names(CWArecurring)[1] <- "Region"
CWArecurring[1,1] <- as.character("Rest of State")
CWArecurring[2,1] <- as.character("OK 2")
CWArecurring$Region2 <- as.character("All of OK")
CWArecurring[2,4] <- as.character("OK 2")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for OK 2
RCRArecurring <- `recurring-violations_All_pg3_OK-2`[-c(1,2,4,5),]
RCRArecurring <- RCRArecurring %>%
  select(CD,Facilities, Percent)
names(RCRArecurring)[1] <- "Region"
RCRArecurring[1,1] <- as.character("Rest of State")
RCRArecurring[2,1] <- as.character("OK 2")
RCRArecurring$Region2 <- as.character("All of OK")
RCRArecurring[2,4] <- as.character("OK 2")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in OK2, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_OK-090220.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_OK-090220.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_OK-2`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_OK-2`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_OK-090220.csv`, `inspectionsper1000_All_pg4_OK-2`)
...
names(`violationsper1000_All_pg4_OK-090220.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_OK-090220.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_OK-2`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_OK-2`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_OK-090220.csv`, `violationsper1000_All_pg4_OK-2`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_OK-2` 
...
vCAAgraph <- ggplot(CAAviolations, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
  scale_color_manual(values=c("#450d54"))+ 
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_OK-2`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_OK-2`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_OK-2`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_OK-2`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_OK-2`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_OK-2`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_OK-2`
...
The GHGRP is an EPA Program mandated by law that requires reporting of major sources and suppliers of GHG emissions in the United States, including from large direct emissions sources, fuel and industrial gas suppliers, and CO2 iORections sites. About 8,000 facilities are required to report their emissions annually, covering 85-90% of total US Annual GHG emissions [EPA](https://www.epa.gov/ghgreporting). By reporting direct sources and suppliers in the same way, the program can misrepresent the geography of emissions, making it appear as if industrial regions have an excess of emissions and suburban areas--where maOR household-level direct emissions occur--have little to no emissions. 
...
GHGs <- `emissions2018_GHGRP_pg4_OK-2`
#GHGs_state <- `emissions2018_GHGRP_pg4_OK-090220.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "OK 2 Emissions"
names(GHGs)[3] <- "OK Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`OK Emissions`-GHGs$`OK 2 Emissions`)
GHGs$`OK 2 Emissions` <- (GHGs$`OK 2 Emissions`/100000)
GHGs$`OK 2 Emissions` <- round(GHGs$`OK 2 Emissions`,2)
GHGs$`OR Emissions` <- (GHGs$`OK Emissions`/100000)
GHGs$`OR Emissions` <- round(GHGs$`OK Emissions`,2)
...
GHG_CD$Region <- "OK"
GHG_OK <- GHGs2[c(1,3)]
names(GHG_OK)[2] <- "Emissions"
GHG_OK$Region <- "OK 2"
GHGs_forgraph <- rbind(GHG_OK, GHG_CD)
...
<a ><img src="markwayne_mullin.png" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2><span style="color:#25539f">Republican</span></h2>
## In office since January 4th, 2007

## Relevant Committee Membership: Member, [Energy and Commerce Committee](https://energycommerce.house.gov/)
<br />
<br />
<div class="text-box-info">
...
1. House Committee on Energy and Commerce, Environment and Climate Change, Health, Oversight and Investigations subcommittees 
