...
```
 <div class="container">
  <img src="MD3_map.png" alt="Snow" style="width:100%;">
...
CAArecurring$Region2 <- as.character("All of MD")
...
CWArecurring$Region2 <- as.character("All of MD")
...
RCRArecurring$Region2 <- as.character("All of MD")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in MD3, were in violation for at least 25% of the last 3 years.</span></h2>
...
GHGs <- `emissions2018_GHGRP_pg4_MD-3`
#GHGs_state <- `emissions2018_GHGRP_pg4_MD-090220.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "MD 3 Emissions"
names(GHGs)[3] <- "MD Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`MD Emissions`-GHGs$`MD 3 Emissions`)
GHGs$`MD 3 Emissions` <- (GHGs$`MD 3 Emissions`/100000)
GHGs$`MD 3 Emissions` <- round(GHGs$`MD 3 Emissions`,2)
GHGs$`OR Emissions` <- (GHGs$`MD Emissions`/100000)
GHGs$`OR Emissions` <- round(GHGs$`MD Emissions`,2)
...
GHG_CD$Region <- "MD"
GHG_MD <- GHGs2[c(1,3)]
names(GHG_MD)[2] <- "Emissions"
GHG_MD$Region <- "MD 3"
GHGs_forgraph <- rbind(GHG_MD, GHG_CD)
...
<a ><img src="john_sarbanes.png" style="float: left; width: 25%; margin-right: 5%" ></a>
