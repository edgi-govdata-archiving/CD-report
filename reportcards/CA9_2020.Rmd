...
filenames = list.files(here("CD_Dirs/CA9"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/CA9", i)))
}

activefacilities <- `active-facilities_All_pg3_CA-9`
...
CA9 <- housemembers[which(housemembers$CD=="CA9"), ]
...
##### California's 9th District; seat held by `r CA9[1,2]` since `r CA9[1,8]` 
 <div class="container">
  <img src="../CD_maps/CA9_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_CA-9`
...
enforcement <- `enforcements_All_pg3_CA-9`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="CA9", "CA9", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="CA9", "CA9", "Other")
...
CA9_dot <- CWAper1000[which(CWAper1000$CD.State=="CA9"), ]
...
ggsave("dotplot_CA9.png", dotplot, path=here("CD_Dirs/CA9"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/CA9/dotplot_CA9.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In CA9, there were <span style="color:#569b5e">`r CA9_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CA 9
CAArecurring <- `recurring-violations_All_pg3_CA-9`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CA 9")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CA 9")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_CA-9`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CA-9`
...
```
# Highlights for California's 9th District
...
names(`inspectionsper1000_All_pg4_CA-9`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA-9`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA-091820.csv`, `inspectionsper1000_All_pg4_CA-9`)
...
inspect1000bar <- ggplot(inspectionsper1000, aes(fill=Region, y=Inspectionsper1000, x=Program, alpha=Program))+
  ...
  scale_y_continuous(expand=c(0,0),limits=c(0,2200))+geom_hline(yintercept=1000, linetype="dashed", color="black")+geom_text(aes(label="Average of 1 inspection per facility",family="Georgia"),x=2,y=1050,color="black",size=4)+
  theme_meg()
...
names(`violationsper1000_All_pg4_CA-9`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA-9`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA-091820.csv`, `violationsper1000_All_pg4_CA-9`)
...
CAAbadactors <- `noncomp_CAA_pg6_CA-9`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_CA-9`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_CA-9`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/CA9"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/CA9"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/CA9"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../CD_Dirs/CA9/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/CA9/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/CA9/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CA-9` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CA-9`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CA-9`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CA-9`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CA-9`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CA-9`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CA-9`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CA-9`
...
<br />
<a ><img src="../CD_images/CA9_rep.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r CA9[1,2]`](`r CA9[1,9]`) (`r CA9[1,3]`)</h2>

## In office since `r CA9[1,6]` `r CA9[1,7]`, `r CA9[1,8]`
...
<h4><span style="padding-left:30px">[`r CA9[1,10]`](`r CA9[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA9[1,12]`](`r CA9[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA9[1,14]`](`r CA9[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r CA9[1,16]`](`r CA9[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/CA9_2020.pdf)
