...
filenames = list.files(here("CD_Dirs/SC3"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/SC3", i)))
}

activefacilities <- `active-facilities_All_pg3_SC-3`
...
SC3 <- housemembers[which(housemembers$CD=="SC3"), ]
...
##### South Carolina's 3rd District; seat held by `r SC3[1,2]` since `r SC3[1,8]` 
 <div class="container">
  <img src="../CD_maps/SC3_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_SC-3`
...
enforcement <- `enforcements_All_pg3_SC-3`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="SC3", "SC3", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="SC3", "SC3", "Other")
...
SC3_dot <- CWAper1000[which(CWAper1000$CD.State=="SC3"), ]
...
ggsave("dotplot_SC3.png", dotplot, path=here("CD_Dirs/SC3"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/SC3/dotplot_SC3.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In SC3, there were <span style="color:#569b5e">`r SC3_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for SC 3
CAArecurring <- `recurring-violations_All_pg3_SC-3`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("SC 3")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("SC 3")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_SC-3`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_SC-3`
...
```
# Highlights for South Carolina's 3rd District
...
names(`inspectionsper1000_All_pg4_SC-091820.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_SC-091820.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_SC-3`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_SC-3`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_SC-091820.csv`, `inspectionsper1000_All_pg4_SC-3`)
...
names(`violationsper1000_All_pg4_SC-091820.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_SC-091820.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_SC-3`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_SC-3`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_SC-091820.csv`, `violationsper1000_All_pg4_SC-3`)
...
CAAbadactors <- `noncomp_CAA_pg6_SC-3`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_SC-3`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_SC-3`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/SC3"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/SC3"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/SC3"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../CD_Dirs/SC3/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/SC3/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/SC3/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_SC-3` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_SC-3`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_SC-3`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_SC-3`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_SC-3`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_SC-3`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_SC-3`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_SC-3`
...
<br />
<a ><img src="../CD_images/SC3_rep.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r SC3[1,2]`](`r SC3[1,9]`) (`r SC3[1,3]`)</h2>

## In office since `r SC3[1,6]` `r SC3[1,7]`, `r SC3[1,8]`
...
<h4><span style="padding-left:30px">[`r SC3[1,10]`](`r SC3[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r SC3[1,12]`](`r SC3[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r SC3[1,14]`](`r SC3[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r SC3[1,16]`](`r SC3[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/SC3_2020.pdf)
