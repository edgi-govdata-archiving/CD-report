...
filenames = list.files(here("../CD_Dirs/IN"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  # name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/IN", i)))
}
...
##### El estado de Indiana; asiento ocupado por `r IN[1,2]` desde `r IN[1,8]` 
 <div class="container">
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="IN", "IN", "otros CDs/\nestados")
...
#removing NY9 and WA5 and MI7, IN, and IA2 and extra CA9
CWAper100 <- CWAper100[-c(37,51,28,14,53,8),]

IN_dot <- CWAper1000[which(CWAper1000$CD.State=="IN"), ]

##THIS CODE WILL NOT WORK FOR NY9 and WA5 and MI7, IN, and IA2. Omit this chunk of code for those districts (here to the end of this r chunk)
...
ggsave("dotplot_IN.png", dotplot, path=here("../CD_Dirs/IN"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/IN/dotplot_IN.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En IN, hubo <span style="color:#569b5e">`r IN_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
```
# ASPECTOS DESTACADOS DEL  ESTADO DE INDIANA
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/IN"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/IN"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/IN"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IN/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IN/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/IN/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
## Ha estado en esta oficina `r IN[1,6]` `r IN[1,7]`, `r IN[1,8]`
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/IN_2020.pdf)
