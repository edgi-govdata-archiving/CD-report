...
filenames = list.files(here("../CD_Dirs/CA"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/CA", i)))
}

activefacilities <- `active-facilities_All_pg3_CA`
...
##### El estado de California; asiento ocupado por `r sen1_name` desde Noviembre 10, 1992 y `r sen2_name` desde Enero 3, 2017
 <div class="container">
  <img src="../State_maps/CA_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- violations_CWA_pg6_CA
...
enforcement <- enforcements_All_pg3_CA
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="CA", "CA", "otros CDs/\nestados")
...
CA_dot <- CWAper1000[which(CWAper1000$CD.State=="CA"), ]
...
ggsave("dotplot_CA.png", dotplot, path=here("../CD_Dirs/CA"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/CA/dotplot_CA.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En CA, hubo <span style="color:#569b5e">`r CA_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for CA
CAArecurring <- `recurring-violations_All_pg3_CA`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("CA")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("CA")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for CA
CWArecurring <- `recurring-violations_All_pg3_CA`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_CA`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_CA`
...
```
# ASPECTOS DESTACADOS DEL ESTADO DE CALIFORNIA
...
names(`inspectionsper1000_All_pg4_CA`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_CA`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_CA`)
...
names(`violationsper1000_All_pg4_CA`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_CA`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_CA`)
...
CAAbadactors <- `noncomp_CAA_pg6_CA`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/CA"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_CA`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/CA"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_CA`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/CA"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/CA/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/CA/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/CA/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_CA` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_CA`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_CA`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_CA`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_CA`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_CA`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_CA`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_CA`
...
<br />
<a ><img src="../CD_images/CA_sen`r senators[1,"sen_class"]`.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r senators$name[1]`](`r senators$wikipedia_url[1]`) (Demócrata)</h2>

## Ha estado en esta oficina desde noviembre 10, del 1992
...
## Nombre del comité: Caucus del Senado de los Estados Unidos sobre el Control Internacional de Estupefacientes 
Jurisdicción: El Caucus del Senado de los Estados Unidos sobre el Control Internacional de Estupefacientes fue creado para "monitorear y alentar el gobierno de los Estados Unidos y los programas privados que buscan expandir la cooperación internacional contra el abuso y el tráfico de narcóticos" y para "monitorear y promover el cumplimiento internacional de los tratados de control de narcóticos". Como organización formal del Senado de los Estados Unidos, el Caucus tiene el estatus de un comité permanente. El Caucus tiene supervisión sobre varias cosas, incluyendo la asistencia internacional contra el narcotráfico y los programas nacionales de prevención y tratamiento de drogas. El Caucus ha organizado varias audiencias a lo largo de los años y ha publicado varios informes sobre la política de control de estupefacientes de EE. UU. [Página web del comité](http://www.drugcaucus.senate.gov/) 
 
## Nombre del comité: Comité Selecto de Inteligencia del Senado 
Jurisdicción: El Comité Selecto de Inteligencia del Senado fue creado para estudiar las actividades y programas de inteligencia del Gobierno de los Estados Unidos e informar al Senado sobre estas actividades y programas de inteligencia. El Comité también es responsable de proponer leyes y proporcionar supervisión legislativa para garantizar que las actividades de inteligencia de los Estados Unidos estén en conformidad con la Constitución y con la ley. [Página web del comité](http://www.intelligence.senate.gov/)
 
## Nombre del comité: Comité de Asignaciones del Senado 
Jurisdicción: El Comité de Asignaciones del Senado es responsable de la legislación que asigna los fondos federales antes de la tesorería. Las asignaciones generalmente se limitan a los niveles establecidos por la Resolución de Presupuesto redactada por el Comité de Presupuesto del Senado. El Comité también tiene responsabilidad sobre las facturas de gastos suplementarios, que a veces se necesitan a la mitad de un año fiscal para compensar los gastos de emergencia. [Página web del comité](http://www.appropriations.senate.gov/) 
Subcomité: Agricultura, Desarrollo Rural, Administración de Alimentos y Medicamentos y Agencias Relacionadas, Rango: 2 
Subcomité: Departamento de Defensa, Rango: 3 
Subcomité: Comercio, Justicia, Ciencia y Agencias Relacionadas, Rango: 3 
Subcomité: Departamento del Interior, Medio Ambiente y Agencias Relacionadas, Rango: 2 
Subcomité: Desarrollo de energía y agua, Rango: 1 
Subcomité: Transporte, Vivienda y Desarrollo Urbano y Agencias Relacionadas, Rango: 4 
...
## Nombre del comité: Comité de la Judicatura del Senado 
Jurisdicción: El Comité de la Judicatura del Senado supervisa el Departamento de Justicia y las agencias bajo la jurisdicción del Departamento, incluyendo la Oficina Federal de Investigaciones y el Departamento de Seguridad Nacional. También juega un papel importante en la consideración de nominaciones y legislación pendiente. Cualquier legislación relacionada con la libertad civil, enmiendas constitucionales, inmigración y naturalización, y varios otros temas es remitido al Comité. [Página web del comité](http://www.judiciary.senate.gov/)
Subcomité: Seguridad Fronteriza e Inmigración, Rango: 2 
Subcomité: Crimen y terrorismo, Rango: 2 
 
## Nombre del comité: Comité de Reglas y Administración del Senado 
Jurisdicción: El Comité de Reglas y Administración del Senado tiene jurisdicción legislativa sobre la organización y funcionamiento del Congreso. Esto incluye asuntos como las reglas y procedimientos del Congreso, la corrupción, la relación del Congreso con las otras ramas del gobierno y la administración general del Senado. El Comité también es responsable de recomendar mejoras en la organización y la operación para fortalecer el Congreso, simplificar sus operaciones, mejorar sus relaciones con otras ramas del Gobierno de los Estados Unidos y permitirle cumplir mejor con sus responsabilidades bajo la Constitución de los Estados Unidos. [Página web del comité](http://www.rules.senate.gov/)
...
# INFORMACIÓN DEL LEGISLADOR
<br />
<a ><img src="../CD_images/CA_sen`r senators[2,"sen_class"]`.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r senators$name[2]`](`r senators$wikipedia_url[2]`) (Demócrata)</h2>

## Ha estado en esta oficina desde el enero 3 del 2017

### [Página de GovTrack](`r senators$govtrack_url[2]`)
...
## Nombre del comité: Comité de Presupuesto del Senado 
Jurisdicción: El Comité de Presupuesto del Senado es responsable de redactar una resolución concurrente sobre el presupuesto para los gastos, los ingresos y los límites de deuda del Congreso. La Resolución de Presupuesto debe ser aprobada por la Cámara y el Senado, pero no necesita ser firmada por el presidente. El Comité también es responsable de hacer cumplir la Resolución del Presupuesto informando a los senadores de las violaciones al presupuesto. El comité también debe trabajar con otros comités, como el Comité de Asignaciones, para prevenir posibles violaciones. [Página web del comité](http://www.budget.senate.gov/) 
 
## Nombre del comité: Comité de Seguridad Nacional y Asuntos Gubernamentales 
Jurisdicción: El Comité de Seguridad Nacional y Asuntos Gubernamentales es el comité de supervisión principal del Senado con una amplia jurisdicción sobre el Departamento de Seguridad Nacional y las operaciones gubernamentales en general. Sus responsabilidades principales incluyen estudiar la eficiencia, economía y eficacia de todas las agencias y departamentos del gobierno federal. Este comité también debe evaluar los efectos de las leyes promulgadas para reorganizar el legislativo y ramas ejecutivas del gobierno; y estudiar las relaciones intergubernamentales entre los EE. UU., en los estados y los municipios, y entre los EE. UU. y las organizaciones internacionales de las que EE. UU. es miembro. [Página web del comité](http://www.hsgac.senate.gov/)
Subcomité: Subcomité Permanente de Investigaciones, Rango: 3 
Subcomité: Supervisión del Gasto federal y la Gestión de emergencias, Rango: 2 
...
## Nombre del comité: Comité de la Judicatura del Senado 
Jurisdicción: El Comité de la Judicatura del Senado supervisa el Departamento de Justicia y las agencias bajo la jurisdicción del Departamento, incluyendo la Oficina Federal de Investigaciones y el Departamento de Seguridad Nacional. También juega un papel importante en la consideración de nominaciones y legislación pendiente. Cualquier legislación relacionada con la libertad civil, enmiendas constitucionales, inmigración y naturalización, y varios otros temas es remitido al Comité. [Página web del comité](http://www.judiciary.senate.gov/) 
Subcomité: la Constitución, Rango: 5 
Subcomité: Propiedad Intelectual, Rango: 7 
...
<hr style="height:3px;border-width:0;color: #C1E0D7;background-color: #C1E0D7; margin-top: 2em">
<h4>INFORME GENERADO EL 12.13.2020<span style="padding-left:350px">PÁGINA 15</span></h4>
...
<hr style="height:3px;border-width:0;color: #C1E0D7;background-color: #C1E0D7; margin-top: 2em">
<h4>INFORME GENERADO EL 12.13.2020<span style="padding-left:350px">PÁGINA 16</span></h4>
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/CA_2020.pdf)
