...
filenames = list.files(here("testing"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("testing", i)))
}
...
```
 <div class="container">
  <img src="KY2_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">Kentucky's 2nd District</div>
...
# Highlights for Kentucky's 2nd District
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_KY-2`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_KY-2`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_KY-2`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for KY 2
CAArecurring <- `recurring-violations_All_pg3_KY-2`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("KY 2")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("KY 2")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in KY2, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_KY-082720.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_KY-082720.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_KY-2`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_KY-2`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_KY-082720.csv`, `inspectionsper1000_All_pg4_KY-2`)
...
names(`violationsper1000_All_pg4_KY-082720.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_KY-082720.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_KY-2`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_KY-2`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_KY-082720.csv`, `violationsper1000_All_pg4_KY-2`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_KY-2` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_KY-2`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_KY-2`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_KY-2`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_KY-2`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_KY-2`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_KY-2`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_KY-2`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_KY-2`
...
GHGs <- `emissions2018_GHGRP_pg4_KY-2`
#GHGs_state <- `emissions2018_GHGRP_pg4_KY-082720.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "KY 2 Emissions"
names(GHGs)[3] <- "OR Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`OR Emissions`-GHGs$`KY 2 Emissions`)
GHGs$`KY 2 Emissions` <- (GHGs$`KY 2 Emissions`/100000)
GHGs$`KY 2 Emissions` <- round(GHGs$`KY 2 Emissions`,2)
...
GHG_CD$Region <- "KY"
GHG_KY <- GHGs2[c(1,3)]
names(GHG_KY)[2] <- "Emissions"
GHG_KY$Region <- "KY 2"
GHGs_forgraph <- rbind(GHG_KY, GHG_CD)
