...
filenames = list.files(here("../CD_Dirs/TX35"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/TX35", i)))
}

activefacilities <- `active-facilities_All_pg3_TX-35`
...
##### El 35$^\circ$ distrito de Texas; asiento ocupado por `r leg_info$name` desde `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/TX35_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_TX-35`
...
enforcement <- `enforcements_All_pg3_TX-35`
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="TX35", "TX35", "otros CDs/\nestados")
...
TX35_dot <- CWAper1000[which(CWAper1000$CD.State=="TX35"), ]
...
ggsave("dotplot_TX35.png", dotplot, path=here("../CD_Dirs/TX35"), scale = 2, width=9, height=5, units=c("cm"))
...
<a ><img src="../CD_Dirs/TX35/dotplot_TX35.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En TX35, hubo <span style="color:#569b5e">`r TX35_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
<br />
...
#ok so We'll put the % in the title for the state, and try to figure out the same for TX 35
CAArecurring <- `recurring-violations_All_pg3_TX-35`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("TX 35")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("TX 35")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_TX-35`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_TX-35`
...
```
# ASPECTOS DESTACADOS DEL 35$^\circ$ DISTRITO DE TEXAS
...
names(`inspectionsper1000_All_pg4_TX-35`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_TX-35`$Region <- as.factor("Este Distrito Congresual")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_TX-112920.csv`, `inspectionsper1000_All_pg4_TX-35`)
...
names(`violationsper1000_All_pg4_TX-35`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_TX-35`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_TX-112920.csv`, `violationsper1000_All_pg4_TX-35`)
...
CAAbadactors <- `noncomp_CAA_pg6_TX-35`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/TX35"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_TX-35`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/TX35"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_TX-35`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/TX35"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX35/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX35/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX35/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_TX-35` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_TX-35`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_TX-35`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_TX-35`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_TX-35`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_TX-35`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_TX-35`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_TX-35`
...
<br />
<a ><img src="../CD_images/TX35_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
### Subcomité: Salud, Rango: 1  <br /> 

### Subcomité: Seleccionar Medidas de Ingresos, Rango: 2
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/TX35_2020.pdf)
