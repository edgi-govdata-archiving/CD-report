...
filenames = list.files(here("../CD_Dirs/ND"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  # name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("../CD_Dirs/ND", i)))
}
...
##### El estado de North Dakota; asiento ocupado por `r ND[1,2]` desde `r ND[1,8]` 
 <div class="container">
...
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="ND", "ND", "otros CDs/\nestados")
...
#removing NY9 and WA5 and MI7, ND, and IA2 and extra CA9
CWAper100 <- CWAper100[-c(37,51,28,14,53,8),]

ND_dot <- CWAper1000[which(CWAper1000$CD.State=="ND"), ]

##THIS CODE WILL NOT WORK FOR NY9 and WA5 and MI7, ND, and IA2. Omit this chunk of code for those districts (here to the end of this r chunk)
...
ggsave("dotplot_ND.png", dotplot, path=here("../CD_Dirs/ND"), scale = 2, width=9, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/ND/dotplot_ND.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>En ND, hubo <span style="color:#569b5e">`r ND_dot[1,5]`</span> violaciones de la Ley de Agua Limpia por cada 100 instalaciones reguladas en 2019.</h3>
...
```
# ASPECTOS DESTACADOS DEL  ESTADO DE NORTH DNDOTA
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("../CD_Dirs/ND"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("../CD_Dirs/ND"), scale = 3, width=7, height=4, units=c("cm"))
}
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("../CD_Dirs/ND"), scale = 3, width=7, height=4, units=c("cm"))
}
RCRAbadactors <- RCRAbadactors[-c(11:20),]
RCRAbadactors$Facility <- RCRAbadactors$FAC_NAME
RCRAbadactors$Facility <- strtrim(RCRAbadactors$Facility, 30)
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/ND/CAAfacilities.png\" style=\"float: left; width: 55%; 
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/ND/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/ND/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**Informes de ECHO:**</font><font size=\"1.5\"><br />"
  }`
...
## Ha estado en esta oficina `r ND[1,6]` `r ND[1,7]`, `r ND[1,8]`
...
[Enlace para descargar la versión PDF de este informe](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/ND_2020.pdf)
