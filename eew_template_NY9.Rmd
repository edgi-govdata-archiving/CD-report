...
filenames = list.files(here("NY9"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("NY9", i)))
}

activefacilities <- `active-facilities_All_pg3_NY-9`
...
```
 <div class="container">
  <img src="ny9_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">New York's 9th District</div>
  <div class="middle-left">Seat held by Yvette Clarke since 2013</div>
</div>
...
<a href="https://www.environmentalenforcementwatch.org/"><img src="eew_logo.png" style="float: left; width: 15%; margin-left: 20%" ></a>
...
# Highlights for New York's 9th District
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_NY-9`
...
effluentgraph <- ggplot(effluentviolations, mapping=aes(x=factor(Year),
  ...
  scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+
  geom_point()+
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NY-9`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_NY-9`
...
enforcementactiongraph <- ggplot(enforcement, mapping=aes(x=factor(Date),y=Count, color=President, group=1))+
  geom_point(show.legend=FALSE)+
  geom_line(show.legend = FALSE)+
  scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+
  labs(y="# Actions", x="Year")+
  ggtitle("Enforcement Actions")+
...
enforcementfinegraph <- enforcement %>% ggplot()+
  geom_point(aes(x=factor(Date),y=Amountinthousands , color=President, group=1), show.legend=FALSE)+
  scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+
  geom_line(aes(x=factor(Date),y=Amountinthousands, color=President, group=1), show.legend = FALSE)+
  labs(y="Fines ($1000s)", x="Year")+
  ggtitle("Enforcement Fines")+
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NY 9
CAArecurring <- `recurring-violations_All_pg3_NY-9`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NY 9")
CAArecurring$Region2 <- as.character("All of NY")
CAArecurring[2,4] <- as.character("NY 9")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for NY 9
CWArecurring <- `recurring-violations_All_pg3_NY-9`[-c(1,3,4,6),]
CWArecurring <- CWArecurring %>%
  select(CD,Facilities, Percent)
names(CWArecurring)[1] <- "Region"
CWArecurring[1,1] <- as.character("Rest of State")
CWArecurring[2,1] <- as.character("NY 9")
CWArecurring$Region2 <- as.character("All of NY")
CWArecurring[2,4] <- as.character("NY 9")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for NY 9
RCRArecurring <- `recurring-violations_All_pg3_NY-9`[-c(1,2,4,5),]
RCRArecurring <- RCRArecurring %>%
  select(CD,Facilities, Percent)
names(RCRArecurring)[1] <- "Region"
RCRArecurring[1,1] <- as.character("Rest of State")
RCRArecurring[2,1] <- as.character("NY 9")
RCRArecurring$Region2 <- as.character("All of NY")
RCRArecurring[2,4] <- as.character("NY 9")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in NY9, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_NY-082720.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NY-082720.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_NY-9`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NY-9`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NY-082720.csv`, `inspectionsper1000_All_pg4_NY-9`)
...
inspect1000bar <- ggplot(inspectionsper1000, aes(fill=Region, y=Inspectionsper1000, x=Program))+
  ...
  geom_rect(fill="#FFD300", alpha=.02, xmin=0, xmax=1.5, ymin=0, ymax=300)+
  geom_rect(fill="#50C878", alpha=.02, xmin=1.5, xmax=2.5, ymin=0, ymax=300)+
  geom_rect(fill="#FFD300", alpha=.02, xmin=2.5, xmax=3.5, ymin=0, ymax=300)+
...
names(`violationsper1000_All_pg4_NY-082720.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NY-082720.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_NY-9`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NY-9`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NY-082720.csv`, `violationsper1000_All_pg4_NY-9`)
...
violations1000bar <- ggplot(violationsper1000, aes(fill=Region, y=violationsper1000, x=Program))+
  ...
  geom_rect(fill="#C21807", alpha=.02, xmin=0, xmax=1.5, ymin=0, ymax=600)+
  geom_rect(fill="#50C878", alpha=.02, xmin=1.5, xmax=2.5, ymin=0, ymax=600)+
  geom_rect(fill="#C21807", alpha=.02, xmin=2.5, xmax=3.5, ymin=0, ymax=600)+
  scale_fill_manual(values=c("#999999" , "#333333", "#d43a69"))+
...
#clean up data
CAAviolations <- `violations_CAA_pg5_NY-9` 
#CAAviolations <- CAAviolations[-c(20),]
CAAviolations$President <- as.character("Bush")
CAAviolations[9:16,3] <- as.character("Obama")
CAAviolations[17:18,3] <- as.character("Trump")



vCAAgraph <- ggplot(CAAviolations, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
  scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_NY-9`
#CAAinspections <- CAAinspections[-c(19),]
CAAinspections$President <- as.character("Bush")
CAAinspections[7:14,3] <- as.character("Obama")
CAAinspections[15:17,3] <- as.character("Trump")

iCAAgraph <- ggplot(CAAinspections, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3,show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Inspections", x="Year")+
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NY-9`
...
eCAAgraph <- ggplot(CAAenforcement, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3,show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Actions", x="Year")+
...
fineCAAgraph <- ggplot(CAAenforcement, mapping=aes(x=factor(Date), y=Amountinthousands, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3)+
  geom_line()+
  labs(y=" Fines($1000s)", x="Year")+
...
#clean up data
CWAviolations <- `violations_CWA_pg6_NY-9`
...
vCWAgraph <- ggplot(CWAviolations, mapping=aes(x=factor(YEAR), y=Count, color=President, group=1))+
  scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3,show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Violations", x="Year")+
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NY-9`
...
iCWAgraph <- ggplot(CWAinspections, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3,show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Inspections", x="Year")+
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NY-9`
...
eCWAgraph <- ggplot(CWAenforcement, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3, show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Actions", x="Year")+
...
fineCWAgraph <- ggplot(CWAenforcement, mapping=aes(x=factor(Date), y=Amountinthousands, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3)+
  geom_line()+
  labs(y=" Fines($1000s)", x="Year")+
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_NY-9`
...
vRCRAgraph <- ggplot(RCRAviolations, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
  scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3,show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Violations", x="Year")+
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NY-9`
...
iRCRAgraph <- ggplot(RCRAinspections, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3,show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Inspections", x="Year")+
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NY-9`
...
eRCRAgraph <- ggplot(RCRAenforcement, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3, show.legend = FALSE)+
  geom_line(show.legend = FALSE)+
  labs(y=" # Actions", x="Year")+
...
fineRCRAgraph <- ggplot(RCRAenforcement, mapping=aes(x=factor(Date), y=Amountinthousands, color=President, group=1))+
   scale_color_manual(values=c("#450d54", "#22908d", "#e56d13"))+ 
  geom_point(size=3)+
  geom_line()+
  labs(y=" Fines($1000s)", x="Year")+
...
The GHGRP is an EPA Program mandated by law that requires reporting of major sources and suppliers of GHG emissions in the United States, including from large direct emissions sources, fuel and industrial gas suppliers, and CO2 iNYections sites. About 8,000 facilities are required to report their emissions annually, covering 85-90% of total US Annual GHG emissions [EPA](https://www.epa.gov/ghgreporting). By reporting direct sources and suppliers in the same way, the program can misrepresent the geography of emissions, making it appear as if industrial regions have an excess of emissions and suburban areas--where many household-level direct emissions occur--have little to no emissions. 
[More info on GHGRP](https://docs.google.com/presentation/d/1oky0P2PSVsURLACzw6i3JaTtaNW_L1y0c2qpkEWBmds/edit?usp=sharing)
</div>
<h2>There are `r activefacilities[4,2] ` 
`r activefacilities[4,1]` facilities</h2>
```{r, fig.height = 4, fig.width = 7, fig.align = "center"}
#clean up data

GHGs <- `emissions2018_GHGRP_pg4_NY-9`
GHGs_state <- `emissions2018_GHGRP_pg4_NY-082720.csv`
GHGs <- cbind(GHGs, GHGs_state)
GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "NY 9 Emissions"
names(GHGs)[3] <- "NY Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`NY Emissions`-GHGs$`NY 9 Emissions`)
GHGs$`NY 9 Emissions` <- (GHGs$`NY 9 Emissions`/100000)
GHGs$`NY 9 Emissions` <- round(GHGs$`NY 9 Emissions`,2)
GHGs$`NY Emissions` <- (GHGs$`NY Emissions`/100000)
GHGs$`NY Emissions` <- round(GHGs$`NY Emissions`,2)
GHGs$WithoutCD <- (GHGs$WithoutCD/100000)
GHGs$WithoutCD <- round(GHGs$WithoutCD,2)
GHGs2 <- GHGs[-c(3)]
GHG_CD <- GHGs2[c(1,2)]
names(GHG_CD)[2] <- "Emissions"
GHG_CD$Region <- "NY"
GHG_NY <- GHGs2[c(1,3)]
names(GHG_NY)[2] <- "Emissions"
GHG_NY$Region <- "NY 9"
GHGs_forgraph <- rbind(GHG_NY, GHG_CD)

GHGgraph <- ggplot(GHGs_forgraph, aes(fill=Region, y=Emissions, x=as.factor(Year)))+
  geom_bar(position="stack", stat="identity")+
  scale_fill_manual(values=c("#333333","#d43a69"))+
  labs(y="Emissions (Millions Metric tons CO2e)", x="")+
...
## Interested in making real world comparisons to these numbers? [Try EPA's Greenhouse Gas Equivalencies Calculator!](https://www.epa.gov/energy/greenhouse-gas-equivalencies-calculator)
...
<a ><img src="yvette_clark.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2><span style="color:#25539f">Democrat</span></h2>
## In office since January 3rd, 2013

## Relevant Committee Membership: Vice Chair, [Energy and Commerce Committee](https://energycommerce.house.gov/)
<div class="text-box-info">
