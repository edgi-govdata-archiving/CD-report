...
filenames = list.files(here("IL9"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("IL9", i)))
}

activefacilities <- `active-facilities_All_pg3_IL-9`
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_IL-9`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_IL-9`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_IL-9`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for IL 9
CAArecurring <- `recurring-violations_All_pg3_IL-9`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("IL 9")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("IL 9")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in IL9, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_IL-090120.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_IL-090120.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_IL-9`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_IL-9`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_IL-090120.csv`, `inspectionsper1000_All_pg4_IL-9`)
...
names(`violationsper1000_All_pg4_IL-090120.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_IL-090120.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_IL-9`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_IL-9`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_IL-090120.csv`, `violationsper1000_All_pg4_IL-9`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_IL-9` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_IL-9`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_IL-9`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_IL-9`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_IL-9`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_IL-9`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_IL-9`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_IL-9`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_IL-9`
...
GHGs <- `emissions2018_GHGRP_pg4_IL-9`
#GHGs_state <- `emissions2018_GHGRP_pg4_IL-090120.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "IL 9 Emissions"
names(GHGs)[3] <- "OR Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`OR Emissions`-GHGs$`IL 9 Emissions`)
GHGs$`IL 9 Emissions` <- (GHGs$`IL 9 Emissions`/100000)
GHGs$`IL 9 Emissions` <- round(GHGs$`IL 9 Emissions`,2)
...
GHG_CD$Region <- "OR"
GHG_IL <- GHGs2[c(1,3)]
names(GHG_IL)[2] <- "Emissions"
GHG_IL$Region <- "IL 9"
GHGs_forgraph <- rbind(GHG_IL, GHG_CD)
