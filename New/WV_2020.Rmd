...
filenames = list.files(here("State_Dirs/WV"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/WV", i)))
}

activefacilities <- `active-facilities_All_pg3_WV`
...
WV <- senators[which(senators$state=="WV"), ]
...
##### West Virginia; seat held by `r WV[1,2]` since `r WV[1,8]` 
 <div class="container">
  <img src="../State_maps/WV_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- violations_CWA_pg6_WV
...
enforcement <- enforcements_All_pg3_WV
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="WV", "WV", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="WV", "WV", "Other")
...
WV_dot <- CWAper1000[which(CWAper1000$CD.State=="WV"), ]
...
ggsave("dotplot_WV.png", dotplot, path=here("State_Dirs/WV"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/WV/dotplot_WV.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In WV, there were <span style="color:#569b5e">`r WV_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for WV
CAArecurring <- `recurring-violations_All_pg3_WV`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("WV")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("WV")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for WV
CWArecurring <- `recurring-violations_All_pg3_WV`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_WV`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_WV`
...
names(`inspectionsper1000_All_pg4_WV`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_WV`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_WV`)
...
names(`violationsper1000_All_pg4_WV`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_WV`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_WV`)
...
CAAbadactors <- `noncomp_CAA_pg6_WV`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_WV`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_WV`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("State_Dirs/WV"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/WV"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/WV"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../State_Dirs/WV/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/WV/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/WV/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_WV` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_WV`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_WV`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_WV`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_WV`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_WV`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_WV`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_WV`
...
<br />
<a ><img src="../CD_images/WV_sen.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r WV[1,2]`](`r WV[1,9]`) (`r WV[1,3]`)</h2>

## In office since `r WV[1,6]` `r WV[1,7]`, `r WV[1,8]`
...
<h4><span style="padding-left:30px">[`r WV[1,10]`](`r WV[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WV[1,12]`](`r WV[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WV[1,14]`](`r WV[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r WV[1,16]`](`r WV[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/WV_2020.pdf)
