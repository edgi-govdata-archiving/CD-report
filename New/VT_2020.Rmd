...
filenames = list.files(here("State_Dirs/VT"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/VT", i)))
}
...
VT <- senators[which(senators$state=="VT"), ]
...
##### Vermont; seat held by `r VT[1,2]` since `r VT[1,8]` 
 <div class="container">
  <img src="../State_maps/VT_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="VT", "VT", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="VT", "VT", "Other")
...
ggsave("dotplot_VT.png", dotplot, path=here("State_Dirs/VT"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/VT/dotplot_VT.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In VT, there were <span style="color:#569b5e">`r VT_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for VT
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for VT
CWArecurring <- `recurring-violations_All_pg3_VT`[-c(1,3),]
...
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/VT"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/VT"), scale = 3, width=7, height=4, units=c("cm"))
...
### No consistent violators of the Clean Air Act are reported in ECHO for Vermont. 
...
<a ><img src="../State_Dirs/VT/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/VT/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<br />
<a ><img src="../CD_images/VT_sen.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r VT[1,2]`](`r VT[1,9]`) (`r VT[1,3]`)</h2>

## In office since `r VT[1,6]` `r VT[1,7]`, `r VT[1,8]`
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/VT_2020.pdf)
