...
filenames = list.files(here("State_Dirs/OR"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/OR", i)))
}

activefacilities <- `active-facilities_All_pg3_OR`
...
OR <- senators[which(senators$state=="OR"), ]
...
##### Oregon; seat held by `r OR[1,2]` since `r OR[1,8]` 
 <div class="container">
  <img src="../State_maps/OR_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- violations_CWA_pg6_OR
...
enforcement <- enforcements_All_pg3_OR
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="OR", "OR", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="OR", "OR", "Other")
...
OR_dot <- CWAper1000[which(CWAper1000$CD.State=="OR"), ]
...
ggsave("dotplot_OR.png", dotplot, path=here("State_Dirs/OR"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/OR/dotplot_OR.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In OR, there were <span style="color:#569b5e">`r OR_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for OR
CAArecurring <- `recurring-violations_All_pg3_OR`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("OR")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("OR")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for OR
CWArecurring <- `recurring-violations_All_pg3_OR`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_OR`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_OR`
...
names(`inspectionsper1000_All_pg4_OR`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_OR`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_OR`)
...
names(`violationsper1000_All_pg4_OR`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_OR`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_OR`)
...
CAAbadactors <- `noncomp_CAA_pg6_OR`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_OR`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_OR`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("State_Dirs/OR"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/OR"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/OR"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../State_Dirs/OR/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/OR/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/OR/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_OR` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_OR`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_OR`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_OR`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_OR`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_OR`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_OR`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_OR`
...
<br />
<a ><img src="../CD_images/OR_sen.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r OR[1,2]`](`r OR[1,9]`) (`r OR[1,3]`)</h2>

## In office since `r OR[1,6]` `r OR[1,7]`, `r OR[1,8]`
...
<h4><span style="padding-left:30px">[`r OR[1,10]`](`r OR[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r OR[1,12]`](`r OR[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r OR[1,14]`](`r OR[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r OR[1,16]`](`r OR[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/OR_2020.pdf)
