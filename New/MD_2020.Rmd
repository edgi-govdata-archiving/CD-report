...
filenames = list.files(here("State_Dirs/MD"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/MD", i)))
}

activefacilities <- `active-facilities_All_pg3_MD`
...
MD <- senators[which(senators$state=="MD"), ]
...
##### Maryland; seats held by `r MD[1,2]` since `r MD[1,8]` and `r MD[2,2]` since `r MD[2,8]`
 <div class="container">
  <img src="../State_maps/MD_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- violations_CWA_pg6_MD
...
enforcement <- enforcements_All_pg3_MD
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="MD", "MD", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="MD", "MD", "Other")
...
MD_dot <- CWAper1000[which(CWAper1000$CD.State=="MD"), ]
...
ggsave("dotplot_MD.png", dotplot, path=here("State_Dirs/MD"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/MD/dotplot_MD.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In MD, there were <span style="color:#569b5e">`r MD_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for MD
CAArecurring <- `recurring-violations_All_pg3_MD`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("MD")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("MD")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for MD
CWArecurring <- `recurring-violations_All_pg3_MD`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_MD`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_MD`
...
names(`inspectionsper1000_All_pg4_MD`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_MD`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_MD`)
...
inspect1000bar <- ggplot(inspectionsper1000, aes(fill=Region, y=Inspectionsper1000, x=Program, alpha=Program))+
  ...
  scale_y_continuous(expand=c(0,0),limits=c(0,1200))+geom_hline(yintercept=1000, linetype="dashed", color="black")+geom_text(aes(label="Average of 1 inspection per facility",family="Georgia"),x=2,y=1500,color="black",size=4)+
  theme_meg()
...
names(`violationsper1000_All_pg4_MD`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_MD`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_MD`)
...
CAAbadactors <- `noncomp_CAA_pg6_MD`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_MD`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_MD`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("State_Dirs/MD"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/MD"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/MD"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../State_Dirs/MD/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/MD/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/MD/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_MD` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_MD`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_MD`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_MD`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_MD`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_MD`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_MD`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_MD`
...
<br />
<a ><img src="../CD_images/MD_sen_ben.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r MD[1,2]`](`r MD[1,9]`) (`r MD[1,3]`)</h2>

## In office since `r MD[1,6]` `r MD[1,7]`, `r MD[1,8]`
...
<h4><span style="padding-left:30px">[`r MD[1,10]`](`r MD[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MD[1,12]`](`r MD[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MD[1,14]`](`r MD[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MD[1,16]`](`r MD[1,17]`)</span></h4>
...
# Legislator Information 
<br />
<a ><img src="../CD_images/MD_sen_chris.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r MD[2,2]`](`r MD[2,9]`) (`r MD[2,3]`)</h2>

## In office since `r MD[2,6]` `r MD[2,7]`, `r MD[2,8]`
...
<h4><span style="padding-left:30px">[`r MD[2,10]`](`r MD[2,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MD[2,12]`](`r MD[2,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MD[2,14]`](`r MD[2,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r MD[2,16]`](`r MD[2,17]`)</span></h4>
...
## Page 2: Highlights from this state
...
## Page 3: This State In Comparison
...
## Page 4: Recent Noncompliance in this State
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/MD_2020.pdf)
