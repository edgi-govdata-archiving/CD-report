...
filenames = list.files(here("State_Dirs/NY"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/NY", i)))
}

activefacilities <- `active-facilities_All_pg3_NY`
...
NY <- senators[which(senators$state=="NY"), ]
...
##### New York; seat held by `r NY[1,2]` since `r NY[1,8]` 
 <div class="container">
  <img src="../State_maps/NY_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- violations_CWA_pg6_NY
...
enforcement <- enforcements_All_pg3_NY
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="NY", "NY", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="NY", "NY", "Other")
...
NY_dot <- CWAper1000[which(CWAper1000$CD.State=="NY"), ]
...
ggsave("dotplot_NY.png", dotplot, path=here("State_Dirs/NY"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/NY/dotplot_NY.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In NY, there were <span style="color:#569b5e">`r NY_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NY
CAArecurring <- `recurring-violations_All_pg3_NY`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NY")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("NY")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for NY
CWArecurring <- `recurring-violations_All_pg3_NY`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_NY`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NY`
...
names(`inspectionsper1000_All_pg4_NY`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NY`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NY`)
...
names(`violationsper1000_All_pg4_NY`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NY`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NY`)
...
CAAbadactors <- `noncomp_CAA_pg6_NY`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_NY`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_NY`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("State_Dirs/NY"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/NY"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/NY"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../State_Dirs/NY/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/NY/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/NY/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_NY` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_NY`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NY`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NY`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NY`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_NY`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NY`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NY`
...
<br />
<a ><img src="../CD_images/NY_sen.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r NY[1,2]`](`r NY[1,9]`) (`r NY[1,3]`)</h2>

## In office since `r NY[1,6]` `r NY[1,7]`, `r NY[1,8]`
...
<h4><span style="padding-left:30px">[`r NY[1,10]`](`r NY[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r NY[1,12]`](`r NY[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r NY[1,14]`](`r NY[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r NY[1,16]`](`r NY[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/NY_2020.pdf)
