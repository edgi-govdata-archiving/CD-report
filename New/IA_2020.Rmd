...
filenames = list.files(here("State_Dirs/IA"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/IA", i)))
}

activefacilities <- `active-facilities_All_pg3_IA`
...
IA <- senators[which(senators$state=="IA"), ]
...
##### Iowa; seat held by `r IA[1,2]` since `r IA[1,8]` 
 <div class="container">
  <img src="../State_maps/IA_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- violations_CWA_pg6_IA
...
enforcement <- enforcements_All_pg3_IA
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="IA", "IA", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="IA", "IA", "Other")
...
IA_dot <- CWAper1000[which(CWAper1000$CD.State=="IA"), ]
...
ggsave("dotplot_IA.png", dotplot, path=here("State_Dirs/IA"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/IA/dotplot_IA.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In IA, there were <span style="color:#569b5e">`r IA_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for IA
CAArecurring <- `recurring-violations_All_pg3_IA`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("IA")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("IA")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for IA
CWArecurring <- `recurring-violations_All_pg3_IA`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_IA`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_IA`
...
names(`inspectionsper1000_All_pg4_IA`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_IA`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_IA`)
...
names(`violationsper1000_All_pg4_IA`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_IA`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_IA`)
...
CAAbadactors <- `noncomp_CAA_pg6_IA`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_IA`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_IA`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("State_Dirs/IA"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/IA"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/IA"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../State_Dirs/IA/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/IA/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/IA/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_IA` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_IA`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_IA`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_IA`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_IA`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_IA`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_IA`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_IA`
...
<br />
<a ><img src="../CD_images/IA_sen.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r IA[1,2]`](`r IA[1,9]`) (`r IA[1,3]`)</h2>

## In office since `r IA[1,6]` `r IA[1,7]`, `r IA[1,8]`
...
<h4><span style="padding-left:30px">[`r IA[1,10]`](`r IA[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r IA[1,12]`](`r IA[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r IA[1,14]`](`r IA[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r IA[1,16]`](`r IA[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/IA_2020.pdf)
