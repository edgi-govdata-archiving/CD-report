...
filenames = list.files(here("State_Dirs/OK"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("State_Dirs/OK", i)))
}

activefacilities <- `active-facilities_All_pg3_OK`
...
OK <- senators[which(senators$state=="OK"), ]
...
##### Oklahoma; seat held by `r OK[1,2]` since `r OK[1,8]` 
 <div class="container">
  <img src="../State_maps/OK_map.png" alt="Snow" style="width:100%;">
  </div>
...
CWAviolations <- violations_CWA_pg6_OK
...
enforcement <- enforcements_All_pg3_OK
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="OK", "OK", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="OK", "OK", "Other")
...
OK_dot <- CWAper1000[which(CWAper1000$CD.State=="OK"), ]
...
ggsave("dotplot_OK.png", dotplot, path=here("State_Dirs/OK"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../State_Dirs/OK/dotplot_OK.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In OK, there were <span style="color:#569b5e">`r OK_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for OK
CAArecurring <- `recurring-violations_All_pg3_OK`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("OK")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("OK")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for OK
CWArecurring <- `recurring-violations_All_pg3_OK`[-c(1,3),]
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_OK`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_OK`
...
names(`inspectionsper1000_All_pg4_OK`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_OK`$Region <- as.factor("State")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_OK`)
...
names(`violationsper1000_All_pg4_OK`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_OK`$Region <- as.factor("State")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_OK`)
...
CAAbadactors <- `noncomp_CAA_pg6_OK`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_OK`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_OK`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("State_Dirs/OK"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("State_Dirs/OK"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("State_Dirs/OK"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../State_Dirs/OK/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/OK/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../State_Dirs/OK/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_OK` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_OK`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_OK`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_OK`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_OK`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_OK`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_OK`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_OK`
...
<br />
<a ><img src="../CD_images/OK_sen.png" style="float: left; width: 25%; margin-right: 5%" ></a>


<h2>[`r OK[1,2]`](`r OK[1,9]`) (`r OK[1,3]`)</h2>

## In office since `r OK[1,6]` `r OK[1,7]`, `r OK[1,8]`
...
<h4><span style="padding-left:30px">[`r OK[1,10]`](`r OK[1,11]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r OK[1,12]`](`r OK[1,13]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r OK[1,14]`](`r OK[1,15]`)</span></h4>

<br />

<h4><span style="padding-left:30px">[`r OK[1,16]`](`r OK[1,17]`)</span></h4>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/OK_2020.pdf)
