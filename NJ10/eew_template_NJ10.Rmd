...
filenames = list.files(here("NJ10"), pattern = ".csv")
file_KYunt = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("NJ10", i)))
}

activefacilities <- `active-facilities_All_pg3_KY-2`
...
```
 <div class="container">
  <img src="NJ10_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">New Jersey's 10th District</div>
  <div class="middle-left">Seat held by Brett Guthrie since 2009</div>
</div>
...
# Highlights for New Jersey's 10th District
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_NJ-10`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NJ-10`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_NJ-10`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NJ 10
CAArecurring <- `recurring-violations_All_pg3_NJ-10`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NJ 10")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("NJ 10")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in NJ10, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_NJ-10`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NJ-10`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NJ-082720.csv`, `inspectionsper1000_All_pg4_NJ-10`)
...
names(`violationsper1000_All_pg4_NJ-10`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NJ-10`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NJ-082720.csv`, `violationsper1000_All_pg4_NJ-10`)
...
violations1000bar <- ggplot(violationsper1000, aes(fill=Region, y=violationsper1000, x=Program))+
  ...
  theme_meg()+
  theme(legend.position = c(0.8, 0.8),axis.text.x = element_text(color =(values=c("#C21807" , "#50C878", "#C21807"))))
...
#clean up data
CAAviolations <- `violations_CAA_pg5_NJ-10` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_NJ-10`
CAAinspections <- CAAinspections[-c(17),]
CAAinspections$President <- as.character("Bush")
CAAinspections[6:13,3] <- as.character("Obama")
CAAinspections[14:16,3] <- as.character("Trump")
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NJ-10`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_NJ-10`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NJ-10`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NJ-10`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_NJ-10`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NJ-10`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NJ-10`
...
GHGs <- `emissions2018_GHGRP_pg4_NJ-10`
#GHGs_state <- `emissions2018_GHGRP_pg4_NJ-082720.csv`
#GHGs <- cbind(GHGs, GHGs_state)
#GHGs <- GHGs[-c(3)]
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "NJ 10 Emissions"
names(GHGs)[3] <- "OR Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`OR Emissions`-GHGs$`NJ 10 Emissions`)
GHGs$`NJ 10 Emissions` <- (GHGs$`NJ 10 Emissions`/100000)
GHGs$`NJ 10 Emissions` <- round(GHGs$`NJ 10 Emissions`,2)
GHGs$`OR Emissions` <- (GHGs$`OR Emissions`/100000)
GHGs$`OR Emissions` <- round(GHGs$`OR Emissions`,2)
...
<a ><img src="brett_guthrie.png" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2><span style="color:#C21807">Republican</span></h2>
## In office since January 6th, 2009
...
1. Health
2. Ranking Member - [Oversights and Investigations](https://energycommerce.house.gov/subcommittees/oversight-and-investigations-116th-congress)
