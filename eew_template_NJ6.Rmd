...
filenames = list.files(here("NJ6"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-*])-.*$","\\1", i)
  assign(name, read.csv(here("NJ6", i)))
}

activefacilities <- `active-facilities_All_pg3_NJ-6`
...
```
 <div class="container">
  <img src="nj6_map.png" alt="Snow" style="width:100%;">
  <div class="top-left">New Jersey's 6th District</div>
  <div class="middle-left">Seat held by Frank Pallone since 1993</div>
</div>
...
# Highlights for New Jersey's 6th District
...
##So, we're going to do the first 3 years of ObaNJ and Trump so average
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `effluent-violations_CWA_pg3_NJ-6`
effluentviolations <- effluentviolations[-c(20),]
effluentchange <- data.frame("ObaNJ Average" =mean(effluentviolations[9:11,2]),
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_NJ-6`
...
#[9:11,2] Obama
# [17:19,2] Trump
enforcement <- `enforcements_All_pg3_NJ-6`
...
#ok so We'll put the % in the title for the state, and try to figure out the same for NJ 6
CAArecurring <- `recurring-violations_All_pg3_NJ-6`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("NJ 6")
CAArecurring$Region2 <- as.character("All of NJ")
CAArecurring[2,4] <- as.character("NJ 6")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for NJ 6
CWArecurring <- `recurring-violations_All_pg3_NJ-6`[-c(1,3,4,6),]
CWArecurring <- CWArecurring %>%
  select(CD,Facilities, Percent)
names(CWArecurring)[1] <- "Region"
CWArecurring[1,1] <- as.character("Rest of State")
CWArecurring[2,1] <- as.character("NJ 6")
CWArecurring$Region2 <- as.character("All of NJ")
CWArecurring[2,4] <- as.character("NJ 6")
...
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for NJ 6
RCRArecurring <- `recurring-violations_All_pg3_NJ-6`[-c(1,2,4,5),]
RCRArecurring <- RCRArecurring %>%
  select(CD,Facilities, Percent)
names(RCRArecurring)[1] <- "Region"
RCRArecurring[1,1] <- as.character("Rest of State")
RCRArecurring[2,1] <- as.character("NJ 6")
RCRArecurring$Region2 <- as.character("All of NJ")
RCRArecurring[2,4] <- as.character("NJ 6")
...
```
<h2><span style="color:#50C878">Under the Clean Water Act, the most well-reported regulation, `r CWArecurring[2,2]` facilities, representing `r CWArecurring[2,3]`% of all regulated facilities in NJ6, were in violation for at least 25% of the last 3 years.</span></h2>
...
names(`inspectionsper1000_All_pg4_NJ-082720.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NJ-082720.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_NJ-6`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_NJ-6`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_NJ-082720.csv`, `inspectionsper1000_All_pg4_NJ-6`)
...
names(`violationsper1000_All_pg4_NJ-082720.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NJ-082720.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_NJ-6`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_NJ-6`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_NJ-082720.csv`, `violationsper1000_All_pg4_NJ-6`)
...
#clean up data
CAAviolations <- `violations_CAA_pg5_NJ-6` 
#CAAviolations <- CAAviolations[-c(20),]
CAAviolations$President <- as.character("Bush")
CAAviolations[9:13,3] <- as.character("Obama")
...
vCAAgraph <- ggplot(CAAviolations, mapping=aes(x=factor(Date), y=Count, color=President, group=1))+
  scale_color_manual(values=c("#450d54", "#22908d"))+ 
  ...
  scale_x_discrete(breaks=c(2004,2008,2012))+
  theme_meg()+
  theme(plot.title = element_text(color="#C21807"), axis.text.x=element_text(size=10),
        axis.title.x = element_blank())

#clean up data
CAAinspections <- `inspections_CAA_pg5_NJ-6`
CAAinspections <- CAAinspections[-c(19),]
CAAinspections$President <- as.character("Bush")
CAAinspections[8:15,3] <- as.character("Obama")
CAAinspections[16:18,3] <- as.character("Trump")
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_NJ-6`
...
#clean up data
CWAviolations <- `violations_CWA_pg6_NJ-6`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_NJ-6`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_NJ-6`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_NJ-6`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_NJ-6`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_NJ-6`
...
GHGs <- `emissions2018_GHGRP_pg4_NJ-6`
names(GHGs)[1] <- "Year"
names(GHGs)[2] <- "NJ 6 Emissions"
names(GHGs)[3] <- "NJ Emissions"
GHGs$WithoutCD <- as.numeric(GHGs$`NJ Emissions`-GHGs$`NJ 6 Emissions`)
GHGs$`NJ 6 Emissions` <- (GHGs$`NJ 6 Emissions`/100000)
GHGs$`NJ 6 Emissions` <- round(GHGs$`NJ 6 Emissions`,2)
GHGs$`NJ Emissions` <- (GHGs$`NJ Emissions`/100000)
GHGs$`NJ Emissions` <- round(GHGs$`NJ Emissions`,2)
...
GHG_CD$Region <- "NJ 6"
GHG_NJ <- GHGs2[c(1,3)]
names(GHG_NJ)[2] <- "Emissions"
GHG_NJ$Region <- "MA"
GHGs_forgraph <- rbind(GHG_NJ, GHG_CD)
...
<a ><img src="frank_Pallone.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>

<h2><span style="color:#25539f">Democrat</span></h2>
## In office since January 5th, 1993

## Relevant Committee Membership: Chair, [Energy and Commerce Committee](https://energycommerce.house.gov/)
<div class="text-box-info">
